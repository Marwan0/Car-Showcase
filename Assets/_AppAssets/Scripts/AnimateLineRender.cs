...
public class AnimateLineRender : MonoBehaviour
{
    ...
    public void _AnimateLineRender(float duration)
    {
        ...
        DOTween.To(() => new Vector3(0, 0, 0), x => customV = x, LineRenderPostions[1], duration)
        .OnUpdate(() =>
{
    ...
    LR.SetPosition(2, customV);
        ...
        Tween tt = null;
        tt = DOTween.To(() => new Vector3(0, 0, 0), x => customV = x, LineRenderPostions[2], duration)
                .SetDelay(1f)
                .From(LineRenderPostions[1])
                .OnUpdate(() =>
                {
                    LR.SetPosition(2, customV);
                    if (tt.IsComplete())
                    {
                        txtComponent.transform.DOLocalMove(EndPos, 1).From(startPos); txtComponent.transform.
                        DOScale(Vector3.one, 1).From(Vector3.zero).OnComplete(() => AnimationComplete.Invoke());
                    }
                });
    }
}
